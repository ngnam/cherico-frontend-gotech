...
export default function ProfileFollow() {
    ...
    async function fetchData() {
        if (authState.user) {
            ...
            let result = await request({ url: `/users/${userId}/follow` });
            if (result.meta.code == 200 && result.data != null && result.data.follows != null) {
                setItems(result.data.follows);
            }
        } else {
            history.push('/')
        }
    ...
    return (
        <div className={`profile-follow ${css.class}`}>
            <Header></Header>
            <div className="page-title">フォロー中のユーザー</div>
            {items.length > 0
                ? (items.map((v, i) => {
