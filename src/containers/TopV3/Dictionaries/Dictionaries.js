...
function Dictionaries({ service }) {
  const perPages = 20
  const { state, addItems, setItems, setCurrentPage, setNoData, setScroll } = useDictionaries()
  ...
  const loadContents = async (page) => {
    ...
    const result = await service.fetchContents(perPages, offset, contentsType.DICTIONARIES)
  ...
  return (
    <div className={`${css.class}`}>
      <div className="title">
        <BlockTitle logo={<img src="/assets/icons/dictionary.svg" />} titleText="辞書" />
      </div>
